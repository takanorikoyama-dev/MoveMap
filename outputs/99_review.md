...
- 「商用展開」: 認証/課金機能なし ✓(DEC-005 個人ツール限定 → **DEC-016 で mode C 公開許可**、ただし商用機能は引き続き対象外)
- 「認証・ユーザー管理」: コード上に user table・session 管理なし ✓(DEC-016 後も mode C は認証なし公開)
- スコープ外機能の混入なし

### 2026-05-30 追記: DEC-016 によるスコープ変更
- DEC-005「個人ツール限定」を DEC-016 で上書き、mode C(完全公開)を解禁
- R-03(法的リスク)を **再オープン**状態に変更
- 派生不変条件候補 INV-BIZ-006/007/008(免責強化・Cookie 同意・利用規約導線)を baseline.md に追加
- 詳細: [`outputs/decisions/DEC-016.md`](decisions/DEC-016.md)
...
| # | 内容 | 状態 |
|---|------|------|
| F1 | 公開拡張する場合のアーキ移行: Streamlit → React + FastAPI(AIR-01 の変更条件) | DEC-016 後も保留(現状は Streamlit Cloud で十分) |
| F2 | Should 指標(医療アクセス / 市区町村粒度 / 口コミ)の段階実装 | 一部 launch 後実施済(public_safety / net_migration 追加、市区町村セクション実装) |
| F3 | Could 指標(気候・カスタム重み付け・時系列グラフ)の検討 | カスタム重み付け実装済 |
| F4 | コード公開時のライセンス選定(LEGAL-02:MIT 推奨で確定) | DEC-016 後 mode C 公開のため要確定 |
| F5 | ARIMA vs Prophet の精度評価対決(TECH-04 持ち越し) | 継続 |

### Re-opened: R-03 法的リスク(DEC-016 によって再オープン, 2026-05-30)

mode C(完全公開)解禁に伴い、Phase 0 専門家レビューで提起された R-03 を再オープン。
対応方針は [`outputs/decisions/DEC-016.md`](decisions/DEC-016.md) §「R-03 の再オープンと対応方針」参照。

主な対応:
- 不動産公正競争規約 → 「公的統計の機械的延長」と明示
- 金融商品取引法 → 「投資助言ではない」明示
- 個人情報保護法 → プライバシーポリシー + Cookie 同意(GA4 導入時)
- 利用規約 → `outputs/legal/terms.md` として整備

派生 INV: INV-BIZ-006(投資/移住助言ではない明示)、INV-BIZ-007(Cookie 同意条件付き)、INV-BIZ-008(法務リンク導線)
